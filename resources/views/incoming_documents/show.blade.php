...
@section('content')
    ...
    @php
        ...
        $currentRecipient = collect($incomingDocument->forwardedRecipients ?? [])
            ->sortByDesc('id')
            ->first(fn ($recipient) => (int) $recipient->user_id === $userId);
        $hasReceivedDocument = $currentRecipient
            ? !is_null($currentRecipient->date_received)
            : !is_null($incomingDocument->date_received);
        $canForward = $isOwner && $hasReceivedDocument;
    ...
    <div class="row g-3">
        <div class="col-lg-3">
            <div class="card">
                ...
                <div class="card-body">
                    <div class="mb-2"><span class="text-secondary small fw-bold">Reference</span>
                        <div>{{ $incomingDocument->document_reference_number }}</div>
                    </div>
                    <div class="mb-2"><span class="text-secondary small fw-bold">Date Received</span>
                        <div>
                            {{ $incomingDocument->date_received ? $incomingDocument->date_received->format('F j, Y g:i A') : '' }}
                        </div>
                    </div>
                    <div class="mb-2">
                        <span class="text-secondary small fw-bold">From</span>
                        <div class="text-uppercase small text-secondary fw-bold">{{ $incomingDocument->document_from_type }}
                        </div>
                        <div>{{ optional($incomingDocument->source)->name }}</div>
                    </div>
                    <div class="mb-2"><span class="text-secondary small fw-bold">Type</span>
                        <div>{{ optional($incomingDocument->type)->name }}</div>
                    </div>
                    <div class="mb-2"><span class="text-secondary small fw-bold">DRN</span>
                        <div>{{ $incomingDocument->drn }}</div>
                    </div>
                    <div class="mb-2"><span class="text-secondary small fw-bold">Subject</span>
                        <div>{{ $incomingDocument->subject }}</div>
                    </div>
                    <div class="mb-2"><span class="text-secondary small fw-bold">Description</span>
                        <div class="small">{{ $incomingDocument->description }}</div>
                    </div>
                    <div class="mb-2"><span class="text-secondary small fw-bold">Status</span>
                        <div class="fw-semibold"><span
                                class="badge bg-success">{{ $incomingDocument->current_status }}</span></div>
                    </div>
                    <div class="mb-2"><span class="text-secondary small fw-bold">Signed By</span>
                        <div>{{ $incomingDocument->signed_by }}</div>
                    </div>
                    <div class="mb-2"><span class="text-secondary small fw-bold">Date Signed</span>
                        <div>{{ $incomingDocument->date_signed ? $incomingDocument->date_signed->format('Y-m-d') : '' }}
                        </div>
                    </div>
                    <div class="mb-2"><span class="text-secondary small fw-bold">Priority</span>
                        <div class="fw-semibold">{{ $incomingDocument->priority_level }}</div>
                    </div>
                    <div class="mb-2"><span class="text-secondary small fw-bold">Deadline</span>
                        <div>{{ $incomingDocument->deadline_date ? $incomingDocument->deadline_date->format('Y-m-d') : '' }}
                        </div>
                    </div>
                    <div class="mb-2">
                        <span class="text-secondary small fw-bold">Days Left to Deadline</span>
                        <div id="deadline_days_left"
                            data-deadline="{{ $incomingDocument->deadline_date ? $incomingDocument->deadline_date->format('Y-m-d') : '' }}">
                        </div>
                    </div>
                    ...
                    <div class="d-flex gap-2 mt-3">
                        @if($isOwner)
                            <a href="{{ route('incoming-documents.edit', $incomingDocument) }}"
                                class="btn btn-sm btn-outline-secondary w-100"><i class="bi bi-pencil"></i> Edit</a>
                        @endif
                    </div>
                </div>
            </div>
                    ...
                    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
                        <div class="modal-content">
                            ...
                            <div class="modal-body p-3">
                                <form action="{{ route('incoming-documents.forward', $incomingDocument) }}" method="POST"
                                    id="forwardForm">
                                    @csrf
                                    @if($errors->any())
                                    ...
                                    <div class="mb-3">
                                        <div class="small text-secondary mb-2 w-50" style="width: 100px !important;">Forward
                                            target</div>
                                        <x-oval-radio-group name="forward_to" :options="[
                    ['value' => 'user', 'label' => 'User'],
                    ['value' => 'group', 'label' => 'Group'],
                ]" :value="(string) old('forward_to', 'user')" aria-label="Forward target" size="sm" :required="true"
                                            id-prefix="forward_to" />
                                    </div>

                                    <div class="mb-3" id="forward_user_wrap">
                                        <label class="form-label small mb-1">Forward to</label>
                                        <select name="forwarded_to_user_id" id="forwarded_to_user_id"
                                    ...
                                    <div class="mb-3 d-none" id="forward_group_wrap">
                                        ...
                                        <select name="forwarded_to_group_id" id="forwarded_to_group_id"
                                            class="form-select form-select-sm"
                                            data-initial="{{ old('forwarded_to_group_id') }}">
                                            <option value="">— Select Group —</option>
                                        </select>

                                        <div class="border rounded-2 p-2 mt-3">
                                            <div class="small text-secondary mb-2">Group routing</div>
                                            <x-oval-radio-group name="group_target_mode" :options="[
                    ['value' => 'group', 'label' => 'Forward to Group'],
                    ['value' => 'staff', 'label' => 'Select Multiple Staff'],
                ]" :value="(string) ((string) old('forward_staff_mode', '0') === '1' ? 'staff' : 'group')" aria-label="Group routing" size="sm" :required="true"
                                                id-prefix="group_target_mode" />
                                        </div>

                                        <input type="hidden" name="forward_staff_mode" id="forward_staff_mode"
                                            value="{{ old('forward_staff_mode', '0') }}">

                                        <div id="staff_picker" class="mt-3 d-none">
                                            <div class="border rounded-2 p-2">
                                                <div class="row g-6">
                                                    <div class="col-6 col-lg-6">
                                                        <label class="form-label small mb-1" for="staff_search">Search
                                                            staff</label>
                                                        <input type="text" id="staff_search"
                                                            class="form-control form-control-sm" autocomplete="off"
                                                            inputmode="search">

                                                        <div class="small text-secondary mt-1" id="staff_hint">Type at least 2
                                                            characters to search.</div>
                                                        <div class="alert alert-danger mt-2 d-none" id="staff_error"
                                                            role="alert"></div>

                                                        <div class="mt-2">
                                                            <div class="list-group border rounded-2 overflow-auto"
                                                                id="staff_results" role="listbox" aria-label="Staff results"
                                                                style="max-height: 320px;"></div>
                                                            <button type="button"
                                                                class="btn btn-sm btn-outline-primary w-100 mt-2 d-none"
                                                                id="staff_load_more">Load More</button>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="col-12 col-lg-5">
                                                        <div
                                                            ...
                                                            <div class="small fw-semibold" aria-live="polite">Selected: <span
                                                                    id="staff_selected_count">0</span> / <span
                                                                    id="staff_selected_limit">20</span></div><br>
                                                            <button type="button" class="btn btn-sm btn-outline-secondary"
                                                                id="staff_clear_all">Clear All</button>
                                                        </div>
                                                        <div class="small text-secondary" style="margin-top: 33px !important">
                                                            Selected staff will not appear in search results.</div>
                                                        <ul class="list-group border rounded-2 overflow-auto mt-2"
                                                            id="staff_selected_list" aria-label="Selected staff"
                                                            style="max-height: 320px;"></ul>
                                                    </div>
                                                </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <textarea name="forward_remarks" rows="3"
                                            class="form-control form-control-sm">{{ old('forward_remarks') }}</textarea>
                                    </div>
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form id="addGroupForm" autocomplete="off" novalidate>
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" id="add_group_name" name="group_name"
                            ...
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal"
                                    id="addGroupCancelBtn">Close</button>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <style>
                        ...
                        #editTrackingLogModal {
                            z-index: 2005;
                        }

                        .edit-log-backdrop.show {
                            z-index: 2000;
                        }
                    </style>
                    ...
                    <div id="tracking_history">
                        ...
                        <ol class="dt-timeline" aria-label="Tracking timeline">
                            @forelse($logs as $log)
                                ...
                                @endphp
                                <li class="dt-step {{ $stateClass }}" style="--dt-delay: {{ min($loop->index * 35, 250) }}ms"
                                    aria-label="Step {{ $stepNumber }}" @if($isActive) aria-current="step" @endif>
                                    ...
                                    <div class="dt-card">
                                        <div class="dt-meta">
                                            ...
                                            <div class="d-flex align-items-center gap-2 flex-wrap justify-content-end">
                                                <div class="text-secondary small">
                                                    {{ $log->action_timestamp ? $log->action_timestamp->format('F j, Y g:i A') : '' }}
                                                </div>
                                                @if($canEditLog && $editMode !== null)
                                                    <button type="button"
                                                        class="btn btn-sm btn-outline-secondary py-0 js-edit-tracking-log"
                                                        data-log-id="{{ (int) $log->id }}" aria-label="Edit log">
                                        ...
                                        @if($log->relatedUser)
                                            <div class="small mt-1">To:
                                                {{ mb_strtoupper((string) $log->relatedUser->name, 'UTF-8') }}</div>
                                        @endif
                                        @if($log->relatedSource)
                                            <div class="small mt-1">To: {{ strtoupper($log->relatedSource->source_type) }} -
                                                {{ $log->relatedSource->name }}</div>
                                        @endif
                                        @if($log->remarks)
                                            @php
                                                $updateChanges = null;
                                            ...
                                            @if($manualUpdate !== null)
                                                ...
                                                @php
                                                    ...
                                                    if (str_contains($t, 'urgent') || str_contains($t, 'immediate') || str_contains($t, 'critical') || str_contains($t, 'error') || str_contains($t, 'fail')) {
                                                        $sev = 'critical';
                                                    } elseif (str_contains($t, 'warn') || str_contains($t, 'delay') || str_contains($t, 'pending')) {
                                                        $sev = 'warning';
                                                    } elseif (str_contains($t, 'success') || str_contains($t, 'completed') || str_contains($t, 'done') || str_contains($t, 'ok')) {
                                                        $sev = 'positive';
                                                    }
                                                ...
                                                <div class="remark-card {{ $card }} mt-2" role="region" aria-label="Manual update">
                                                    <div class="remark-head">
                                                        ...
                                                        <div class="remark-badges">
                                                            ...
                                                            @if(trim($rfName) !== '')
                                                                <span
                                                                    class="remark-chip chip-info">{{ strtoupper((string) $rfType) }}</span>
                                                            @endif
                                                        </div>
                                                    </div>
                                                    <div class="remark-body">
                                                        @if(trim($rfName) !== '')
                                                            <div class="small">{{ $rfLabel }} <span
                                                                    class="badge text-bg-primary">{{ strtoupper((string) $rfType) }} -
                                                                    {{ $rfName }}</span></div>
                                                        @endif
                                                        @if(trim($updateText) !== '')
                                                    ...
                                                    <div class="remark-body">
                                                        ...
                                                        @else
                                                            @foreach($updateChanges as $c)
                                                                <div
                                                                    class="d-flex flex-wrap gap-2 align-items-start py-1 {{ !$loop->last ? 'border-bottom' : '' }}">
                                                                    <div class="badge text-bg-secondary">{{ $c['field'] ?? '' }}</div>
                                                                    <div class="small">
                                                                        <span class="text-secondary">from</span>
                                                                        <span
                                                                            class="font-monospace">{{ array_key_exists('old', $c ?? []) ? json_encode($c['old'], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) : 'null' }}</span>
                                                                        <span class="text-secondary mx-1">to</span>
                                                                        <span
                                                                            class="font-monospace">{{ array_key_exists('new', $c ?? []) ? json_encode($c['new'], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) : 'null' }}</span>
                                                                    </div>
                                                                </div>
                                                            @endforeach
                                                        @endif
                                                            ...
                                                            <details class="mt-2">
                                                                ...
                                                                <div class="mt-2 d-flex flex-wrap gap-1">
                                                                    @foreach($recipients as $r)
                                                                        <span
                                                                            class="badge text-bg-primary">{{ mb_strtoupper((string) ($r['name'] ?? ''), 'UTF-8') }}</span>
                                                                    @endforeach
                                                                </div>
                                                            </details>
                                                        @endif
                                                ...
                                                <div class="remark-card remark-positive mt-2" role="region" aria-label="Inbox received">
                                                    ...
                                                    <div class="remark-body">
                                                        @if($recipientUserName !== '')
                                                            <div class="small text-secondary">Recipient:
                                                                {{ mb_strtoupper($recipientUserName, 'UTF-8') }}</div>
                                                        @endif
                                                        @if(trim($displayBehalfName) !== '')
                                                            <div class="small text-secondary">
                                                                Received in behalf:
                                                                <span
                                                                    class="badge badge-pink">{{ mb_strtoupper($displayBehalfName, 'UTF-8') }}</span>
                                                            </div>
                                                        @endif
                                                        @if($documentTypeName !== '')
                                                            <div class="small text-secondary">Type:
                                                                {{ mb_strtoupper($documentTypeName, 'UTF-8') }}</div>
                                                        @endif
                                                    </div>
                                                </div>
                                            @else
                            ...
                            @if(count($futureStatuses) > 0)
                                ...
                                @endphp
                                <li class="dt-step dt-future" style="--dt-delay: {{ min(($stepNumber - 1) * 35, 250) }}ms"
                                    aria-label="Upcoming step {{ $stepNumber }}">
                                    <div class="dt-rail" aria-hidden="true">
                                        <span class="dt-indicator">{{ $stepNumber }}</span>
                                    </div>
                                    <div class="dt-card">
                                            ...
                                            <div class="dt-title">
                                                ...
                                                @if(($canAddUpdate ?? false) === true || (bool) ($isAdminUser ?? false))
                                                    <button type="button" class="btn btn-sm btn-secondary ms-2"
                                                        id="btnAddUpdate">Add Update</button>
                                                @endif
                                            </div>
                                        </div>
                                        {{-- @if($nextStatus !== '')
                            ...
                            <div class="mb-3">
                                <div class="row g-2 align-items-start">
                                    <div class="col-12 col-md-6">
                                        <label class="form-label">Forwarded To <span class="text-danger ms-1"
                                                aria-hidden="true">*</span><span class="visually-hidden">required</span></label>
                                    </div>
                                    <div class="col-12 col-md-6">
                                        <label class="form-label">Document Returned From <span class="text-danger ms-1"
                                                aria-hidden="true">*</span><span class="visually-hidden">required</span></label>
                                    </div>
                                    <div class="col-12">
                                        <div class="row g-2">
                                            <div class="col-12 col-md-6">
                                                <div class="d-flex flex-column gap-2" role="radiogroup"
                                                    aria-label="Document To">
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="radio"
                                                            name="manual_update_party_choice" id="update_to_section"
                                                            value="to_section" required>
                                                        <label class="form-check-label" for="update_to_section">Section</label>
                                                    </div>
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="radio"
                                                            name="manual_update_party_choice" id="update_to_staff"
                                                            value="to_staff">
                                            ...
                                            <div class="col-12 col-md-6">
                                                <div class="d-flex flex-column gap-2" role="radiogroup"
                                                    ...
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="radio"
                                                            name="manual_update_party_choice" id="update_from_section"
                                                            value="from_section" checked>
                                                        <label class="form-check-label"
                                                            for="update_from_section">Section</label>
                                                    </div>
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="radio"
                                                            name="manual_update_party_choice" id="update_from_staff"
                                                            value="from_staff">
                                ...
                                <div class="mb-3">
                                    <label for="return_from_document_source_id" class="form-label"><span
                                            id="return_from_label_text">Returned from</span> <span class="text-danger ms-1"
                                            aria-hidden="true">*</span><span class="visually-hidden">required</span></label>
                                    <select name="return_from_document_source_id" id="return_from_document_source_id"
                                        class="form-select" required>
                                        <option value="">Select returned from</option>
                                        @foreach($returnFromSources as $s)
                                            <option value="{{ $s->id }}" data-type="{{ $s->source_type }}">
                                                {{ strtoupper($s->source_type) }} - {{ $s->name }}</option>
                                        @endforeach
                                    </select>
                                    <div class="invalid-feedback d-block" data-feedback-for="return_from_document_source_id">
                                    </div>
                                </div>

                                <div class="mb-0">
                                    <label for="update_text" class="form-label">Type your update in this document <span
                                            class="text-danger ms-1" aria-hidden="true">*</span><span
                                            class="visually-hidden">required</span></label>
                                    <textarea name="update_text" id="update_text" rows="6" class="form-control"
                                        required></textarea>
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <a class="btn btn-sm btn-outline-primary" href="{{ $attachmentUrl }}" target="_blank"
                    ...
                    <div class="modal-body p-2 p-md-3">
                        ...
                        @elseif(in_array($attachmentExt, ['png', 'jpg', 'jpeg', 'gif', 'webp', 'bmp', 'svg'], true))
                            <img src="{{ $attachmentUrl }}" alt="Attachment" class="img-fluid w-100"
                                style="max-height: 75vh; object-fit: contain;">
                        @else
                            <iframe src="{{ $attachmentUrl }}" title="Attachment"
    ...
    <div class="modal fade" id="editTrackingLogModal" tabindex="-1" aria-labelledby="editTrackingLogModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header">
