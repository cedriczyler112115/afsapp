...
@section('title', '4Ps AFS-IS - Stock Out (Issuance)')
    ...
    $(document).ready(function() {
        ...
        $(document).on('change', '.issuance-checkbox', function() {
            if (!$(this).prop('checked')) {
                ...
                if ($('.issuance-checkbox:checked').length === $('.issuance-checkbox').length) {
        ...
        function togglePrintButton() {
            if ($('.issuance-checkbox:checked').length > 0) {
                $('#print-issuances').prop('disabled', false);
            } else {
                $('#print-issuances').prop('disabled', true);
            }
        }
            ...
            $.ajax({
                ...
                success: function(response) {
                    if(response.success) {
                        ...
                        toastr.success('Issuances grouped successfully');
                        fetchStockTransactions(); // Reload table
        ...
        function fetchStockTransactions(page = 1) {
            ...
            $.ajax({
                url: "{{ route('stock-out.index') }}?page=" + page,
                type: "GET",
                data: { 
                    per_page: perPage, 
                    search: search,
        ...
        $('#reset-filters').on('click', function() {
            ...
            $('#per_page').val('10'); // Reset to default 10 or keep as is? Let's assume 10.
        ...
        $(document).on('click', '.view-details', function() {
            let id = $(this).data('id');
            let url = "{{ route('stock-out.show', ':id') }}".replace(':id', id);
